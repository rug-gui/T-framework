...
root.geometry('+75+75') # set new geometry the + 75 + 75 is where it starts on the screen
if os.path.isfile(os.path.curdir+'/assets/favicon/dark.ico'):
    iconpath=os.path.curdir +'/assets/favicon/dark.ico'
    root.iconbitmap(os.path.curdir+"/assets/favicon/dark.ico") # to show your own icon 
...
if os.path.isfile(iconpath):
    ...
    darkicon = ImageTk.PhotoImage(Image.open(iconpath).resize([20,20]))
    icon = Label(title_bar, image=darkicon,height=20,width=20, bg=RGRAY,bd=0,fg='white',font=("helvetica", 10),highlightthickness=0)
    title_bar_title.pack_forget()
    icon.pack(side=LEFT, padx=10)
    title_bar_title.pack(side=LEFT,expand=1,fill=X,padx=10)
...
resizex_widget_left = Frame(window,bg=DGRAY,cursor='sb_h_double_arrow')
resizex_widget_left.pack(side=LEFT,ipadx=3,fill=Y)
...
resizex_widget_left.bind("<B1-Motion>",resizex_left)
...
resizey_widget.pack(side=BOTTOM,ipadx=4,fill=X)
resizey_widget_top = Frame(window,bg=DGRAY,cursor='sb_v_double_arrow')
resizey_widget_top.pack(side=TOP,ipadx=4,fill=X)
def resizey(event):
...
resizey_widget_top.bind("<B1-Motion>",resizey)
...
root.tk_setPalette(background=DGRAY, foreground='WHITE',
